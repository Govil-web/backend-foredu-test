package com.foroescolar.controllers.calificacion;
...
public class CalificacionController {
    ...
    public ResponseEntity<ApiResponseDto<CalificacionDTO>> saveCalificacion
        ...
        if(user.rol().equals("PROFESOR")){
            ...
            return new ResponseEntity<>(new ApiResponseDto<>( false,"No tiene permisos para acceder a esta opción",null),HttpStatus.UNAUTHORIZED);
        }
    ...
    public ResponseEntity<ApiResponseDto<CalificacionDTO>> findAll() {
        try {
            ...
            return new ResponseEntity<>(new ApiResponseDto<>(true,"Búsqueda Exitosa", list),HttpStatus.CREATED);

        } catch (ApplicationException e){
            throw new ApplicationException(""," Ha ocurrido un error en el servidor: ", e.getHttpStatus());
        }
    }
    ...
    public ResponseEntity<ApiResponseDto<CalificacionDTO>> findByEstudianteId(@PathVariable("idEstudiante") Long idEstudiante) {
        ...
        return new ResponseEntity<>(new ApiResponseDto<>(true, "Búsqueda exitosa", response), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<ApiResponseDto<CalificacionDTO>>obtenerPorMateria(@RequestParam("materia") String materia){
       MateriaEnum materiaEnum= MateriaEnum.valueOf(materia);
        if(materiaEnum.compareTo(MateriaEnum.values()[0])==0){
            throw new ApplicationException("", "La materia no registra calificaciones", HttpStatus.NOT_FOUND);
        }
